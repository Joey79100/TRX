...
static void M_ApplyUniforms(GFX_3D_RENDERER *const renderer)
{
    ...
    GFX_GL_Program_Uniform1i(
        &renderer->program, renderer->loc_dithering_enabled,
        renderer->use_dithering);
...
GFX_3D_RENDERER *GFX_3D_Renderer_Create(void)
{
    ...
    renderer->loc_dithering_enabled =
        GFX_GL_Program_UniformLocation(&renderer->program, "ditheringEnabled");
    ...
    const size_t light_map_size)
{
    ...
    if (light_map != nullptr) {
        ...
        glTexImage2D(
            GL_TEXTURE_2D, 0, GL_RED, palette_size, light_map_size, 0, GL_RED,
            GL_UNSIGNED_BYTE, light_map);
...
void GFX_3D_Renderer_SetDithering(
    GFX_3D_RENDERER *const renderer, const bool is_enabled)
{
    ASSERT(renderer != nullptr);
    if (renderer->use_dithering == is_enabled) {
        return;
    }
    renderer->use_dithering = is_enabled;
    M_Flush(renderer);
}
